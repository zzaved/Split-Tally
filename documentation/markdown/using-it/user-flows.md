...
![The voice picker on a cream background, four dark spheres arranged two by two, each glowing in a different blue, with the names COBALT, INDIGO, ULTRAMARINE and CERULEAN in letter-spaced capitals beneath them](../static/img/screens/voice-picker.png)
...
![The onboarding screen, a large aurora sphere above a transcript where the assistant has asked what to call the user and confirmed the answer in one sentence, with Yes and Fix that chips beneath it and a text input reading "or type instead"](../static/img/screens/onboarding-talk.gif)
...
![A group page for Barcelona Trip, the member balances in serif figures across the top, the expense form with a shares split and a live remainder on the left, and on the right a settle up form with the outstanding amount prefilled and a card listing who pays whom](../static/img/screens/group-page.png)
...
![A review table of transactions read from a bank statement screenshot, each row with a checkbox, an editable date, description, direction and amount, and two rows greyed with an "Already tallied" badge and their checkboxes cleared](../static/img/screens/import-review.png)
...
![The voice sheet in check-in mode, the header reading "Weekly tally", the orb above a conversation in which the assistant asks about cash spending and the user answers, ending with a two sentence recap of the week](../static/img/screens/weekly-tally.gif)
...
![The score page showing an open cobalt ring with the number 60 in its gap, a list of the score's component lines with their point values, a "How to move it" section, and a dated log of settlements and open debts with the days each took](../static/img/screens/score.png)
