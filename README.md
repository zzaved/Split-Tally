...
Create a Conversational AI agent, paste the system prompt below into it, and register the thirteen
...
session credentials), `Voices: read`, and `speech_to_text` (the realtime token the guided form
filler mints in `app/api/voice/scribe-token/route.ts`). Nothing else.
...
> get_spending. Anything about the Tally Score → get_score. Payments received →
...
| `get_spending` | `group_name?`, `days?` | What was spent, which is a different question from who owes whom |
| `get_score` | `person_name?` | The Tally Score with its reading, not the bare number |
